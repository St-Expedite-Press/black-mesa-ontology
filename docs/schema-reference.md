...
THE GRAPH BOUNDARY. Raw spectral imagery is never tiled into the graph. At ten thousand farms over five years the correct architecture is on the order of 373 million triples; with imagery in the graph it is on the order of 248 billion, a factor of roughly 665. The failure is not gradual - query planning, indexing, and backup economics all change character - so it cannot be deferred and corrected later by adding hardware.
...
- **`(SPARQL)`** — GRAPH BOUNDARY VIOLATION: imagery must be referenced by URI and checksum, never tiled into the graph. Per-tile or per-pixel triples inflate the store by roughly 665x, and the failure is not gradual - query planning, indexing and backup economics all change character.
